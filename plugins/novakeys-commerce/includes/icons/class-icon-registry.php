<?php
/**
 * NovaKeys icon registry — SVG sprite + render helpers.
 *
 * Mirrors the design bundle's icons.jsx (44 icons). All inner SVG markup
 * is verbatim from the design — strokes/geometry intact. Icons use
 * `currentColor` so colour follows CSS `color`.
 *
 * Templates render icons three ways:
 *   echo nk_icon( 'truck' );                  // standalone <svg>
 *   echo nk_icon_use( 'truck' );              // <use href="#ngrd-icon-truck"/>
 *   echo nk_icon_sprite();                    // emit the sprite once per page
 *
 * Sprite auto-emits in `wp_footer` when `nk_redesign_active_phases()` (or
 * its legacy `ng_*` alias) returns a truthy phase set; otherwise templates
 * must call `nk_icon_sprite()` themselves.
 *
 * @package NovaKeys\Commerce\Icons
 * @since   0.1.0
 */

namespace NovaKeys\Commerce\Icons;
...
final class Icon_Registry {
	 ...
	 */
	private static ?Icon_Registry $instance = null;

	/**
	 * Static icon catalogue cache.
	 *
	 * @since 0.1.0
	 ...
	 */
	public static function instance(): Icon_Registry {
	...
	/**
	 * Wire the wp_footer auto-emit hook.
	 *
	 * @since 0.1.0
	 * @return void
	 */
	public function register_hooks(): void {
		add_action( 'wp_footer', array( $this, 'maybe_auto_emit_sprite' ), 1 );
	}

	/**
	 * Return the icon catalogue. Filterable via `nk_icons_catalog`.
	 *
	 * @since 0.1.0
	...
	public function catalog(): array {
		...
		/**
		 * Filter the icon catalogue. Allow themes / extensions to add or
		 * override icons. The value of each entry must be the inner SVG
		 * markup that will sit inside <symbol viewBox="0 0 24 24">.
		 *
		 * @since 0.1.0
		 *
		 * @param array<string, string> $icons Icon name → inner SVG.
		 */
		$this->catalog_cache = (array) apply_filters( 'nk_icons_catalog', $icons );
		return $this->catalog_cache;
	}

	/**
	 * Render one icon as a self-contained <svg>.
	 *
	 * Use this when the sprite may not be present yet (e.g. inside an
	 * email or a server-side render before wp_footer fires).
	 *
	 * @since 0.1.0
	 *
	 * @param string $name        Icon name. Falls back to `close` if unknown.
	 * @param int    $size        Icon size in px. Default 20.
	 * @param string $extra_class Additional CSS class names. Default empty.
	 * @return string Safe HTML <svg> element.
	 */
	public function render( string $name, int $size = 20, string $extra_class = '' ): string {
		$icons = $this->catalog();
		if ( ! isset( $icons[ $name ] ) ) {
			$name = 'close';
		}
		...
		return sprintf(
			...
			$size,
			$size,
			$icons[ $name ] // Literal SVG markup, no user input.
		);
	}

	/**
	 * Build the inline sprite (one <svg> with N <symbol> children).
	 *
	 * Pages that prefer `<use href="#ngrd-icon-name">` references should
	 * emit this once, then use {@see use_ref()} to reference icons.
	 *
	 * @since 0.1.0
	 * @return string Safe HTML <svg> sprite.
	 */
	...
	/**
	 * Reference an icon via the sprite. Lighter than {@see render()} when
	 * the same icon repeats many times on a page.
	 *
	 * @since 0.1.0
	 *
	 * @param string $name        Icon name. Falls back to `close` if unknown.
	 ...
	 * @return string Safe HTML <svg> element.
	 */
	public function use_ref( string $name, int $size = 20, string $extra_class = '' ): string {
		$icons = $this->catalog();
		if ( ! isset( $icons[ $name ] ) ) {
			$name = 'close';
		}
	...
	/**
	 * Auto-emit the sprite once in the footer when a redesign phase is
	 * active. Idempotent — multiple invocations are no-ops after the
	 * first emit.
	 *
	 * @since 0.1.0
	 * @return void
	 */
	public function maybe_auto_emit_sprite(): void {
		static $emitted = false;
		if ( $emitted ) {
			return;
		}
		$active = false;
		if ( function_exists( 'nk_redesign_active_phases' ) ) {
			$active = ! empty( nk_redesign_active_phases() );
		} elseif ( function_exists( 'ng_redesign_active_phases' ) ) {
			$active = ! empty( ng_redesign_active_phases() );
		}
		if ( ! $active ) {
			return;
		}
		$emitted = true;
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped — sprite is fully literal SVG, no user input.
		echo $this->sprite();
	}
}

/**
 * Render one icon as a standalone <svg>.
 *
 * @since 0.1.0
 *
 * @param string $name        Icon name.
 * @param int    $size        Icon size in px. Default 20.
 * @param string $extra_class Extra CSS class names. Default empty.
 * @return string Safe HTML.
 */
function nk_icon( string $name, int $size = 20, string $extra_class = '' ): string {
	return Icon_Registry::instance()->render( $name, $size, $extra_class );
}

/**
 * Reference an icon via the page sprite.
 *
 * @since 0.1.0
 *
 * @param string $name        Icon name.
 ...
 * @return string Safe HTML.
 */
function nk_icon_use( string $name, int $size = 20, string $extra_class = '' ): string {
	return Icon_Registry::instance()->use_ref( $name, $size, $extra_class );
}

/**
 * Render the inline icon sprite.
 *
 * @since 0.1.0
 * @return string Safe HTML.
 */
function nk_icon_sprite(): string {
	return Icon_Registry::instance()->sprite();
}

/**
 * Get the raw icon catalogue (name → inner SVG).
 *
 * @since 0.1.0
 * @return array<string, string>
 */
function nk_icons_catalog(): array {
	return Icon_Registry::instance()->catalog();
}

Icon_Registry::instance()->register_hooks();
